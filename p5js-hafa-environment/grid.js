...
class Grid {
	...
	init(initOptions) {
		...
		this.environment = initOptions.environment ? initOptions.environment : null;
	}
	...
	renderParticles(options) {
		this.particles.forEach((agent) => {
			if (this.environment && this.environment.contains(agent)) {
				console.log("environment contains agent...");
				agent.applyEnvironment(this.environment);
			}
			agent.update();
			agent.draw();
			checkEdges(
